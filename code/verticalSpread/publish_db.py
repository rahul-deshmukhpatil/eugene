...
#{
	...
	try:
		cursor.execute('''INSERT INTO %s
			VALUES (%d, %f, %f)''' %(row[0], row[1], row[2], row[3]))	
	except:	
...
#{
	...
	underlying = float(row[13].replace(',','').replace('-', '0'))

	publish_future(cursor, [symbol, openEpoch, op, op - ltp + underlying])
	publish_future(cursor, [symbol, highEpoch, hp, hp - ltp + underlying])
	publish_future(cursor, [symbol, lowEpoch, lp, lp - ltp + underlying])
	publish_future(cursor, [symbol, closeEpoch, cp, cp - ltp + underlying])
	publish_future(cursor, [symbol, ltpEpoch, ltp, ltp - ltp + underlying])
